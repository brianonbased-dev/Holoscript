...
export function registerNeuralLinkHandlers(runtime: any) {
  runtime.on('neural_link_execute', async (payload: { nodeId: string, prompt: string }) => {
    const { getHoloScriptExecutor } = await import('../../services/master-portal/training/HoloScriptExecutor');
    ...
    try {
      ...
      const result = await (executor as any).executeWithLocalInference('neural-session', payload.prompt);
