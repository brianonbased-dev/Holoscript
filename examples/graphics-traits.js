...
import { MaterialTrait, LightingTrait, RenderingTrait, MATERIAL_PRESETS, LIGHTING_PRESETS, } from '@holoscript/core';
/**
...
export function setupStudioLighting() {
    ...
    // Key light (main)
    lighting.createDirectionalLight({ x: 0.8, y: 1, z: 0.6 }, // direction (from upper left)
    { r: 1, g: 0.95, b: 0.9 }, // warm white
    1.2, // strong intensity
    true);
    // Fill light (balance shadows)
    lighting.createPointLight({ x: -0.8, y: 0.5, z: 0.4 }, // opposite side
    { r: 0.7, g: 0.7, b: 0.8 }, // cool blue
    0.6, // medium intensity
    30, // 30 unit range
    false);
    // Back light (rim light for separation)
    lighting.createSpotLight({ x: 0, y: 0.5, z: -1 }, // behind object
    ...
    lighting.setAmbientLight({ r: 0.8, g: 0.85, b: 0.9 }, // sky color (light blue)
...
export class DayNightLighting {
    ...
    initializeDay() {
        // Bright daylight
        this.sunId = this.lighting.createDirectionalLight({ x: 0.5, y: 1, z: 0.5 }, { r: 1, g: 0.95, b: 0.8 }, // warm sunlight
        1.5, // bright
        true);
    }
 ...
 * - Performance tuning
 */
export function setupRenderingForPlatform(platform) {
