...
export async function packAsset(inputDir: string, outputPath: string | undefined, verbose: boolean): Promise<void> {
...
export async function unpackAsset(inputPath: string, outputDir: string | undefined, verbose: boolean): Promise<void> {
...
export async function inspectAsset(inputPath: string, verbose: boolean): Promise<void> {
  const absInput = path.resolve(inputPath);
  ...
  if (scriptLines.length > 10) console.log(`\x1b[2m... (${scriptLines.length - 10} more lines)\x1b[0m`);
