...
<body class="theme-${this.options.theme}">
  <header>
    ...
    <div class="file-info">
      <span class="old-file">📄 ${this.escapeHtml(result.files.old)}</span>
      <span class="arrow">↔</span>
      <span class="new-file">📄 ${this.escapeHtml(result.files.new)}</span>
    </div>
  </header>
  ...
  <footer>
    <p>Generated by HoloScript Visual Diff • ${new Date().toISOString()}</p>
  </footer>
  ...
  private buildSummary(result: SemanticDiffResult): string {
    const stats = [
      { type: 'added', icon: '➕', color: '#22c55e' },
      { type: 'removed', icon: '➖', color: '#ef4444' },
      { type: 'modified', icon: '📝', color: '#f59e0b' },
      { type: 'renamed', icon: '🔄', color: '#3b82f6' },
      { type: 'moved', icon: '📦', color: '#8b5cf6' },
    ];
